...
f = codecs.open("../resource/book_grammars/test.fcfg", mode="w", encoding="utf-8")
...
def pm2fcfg(phrase):  ## phrase - это словосочетание, которое мы разбираем
    f = codecs.open("../resource/book_grammars/test.fcfg", mode="a", encoding="utf-8")
    for x in phrase:    #строка
        ...
        for y in a: # y - объект, а - лист
            if (y.tag.POS == "NOUN") or (y.tag.POS == "ADJF") or (y.tag.POS == "PRTF"):
                ...
                strk = str(y.tag.POS) + "[C=" + str(y.tag.case) + ", NF=u'" + str(y.normal_form) + "'] -> '" + str(
                    y.word) + "'\n"
                f.writelines(strk)
            elif (y.tag.POS == "ADVB") or (y.tag.POS == "GRND") or (y.tag.POS == "COMP") or (y.tag.POS == "PRED") or (
                    y.tag.POS == "PRCL") or (y.tag.POS == "INTJ"):
