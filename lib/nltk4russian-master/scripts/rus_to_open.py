# -*- coding: utf-8 -*-
"""RusCorpora -> OpenCorpora конвертация тегов.
В качестве входного аргумента приводится один анализ RNC (количество тегов), результатом является преобразованный OC «анализ».
Этот скрипт не обрабатывает некоторые случаи, например, связанные с семантическими тегами.
...
def parse_rules(rules_file):
    """Разобрать файл со списком правил и вернуть упорядоченный словарь.
...
def convert_tags(rules_dict, original_list):
    """Преобразовать данный анализ и вернуть набор соответствующих тегов OC (без определенного порядка).
